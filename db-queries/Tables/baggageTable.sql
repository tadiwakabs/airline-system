CREATE TABLE `Baggage` (
       ...
       `ticketCode` varchar(30) DEFAULT NULL,
       PRIMARY KEY (`baggageId`),
       KEY `passengerId` (`passengerId`),
       KEY `fk_baggage_ticket` (`ticketCode`),
       CONSTRAINT `Baggage_ibfk_1` FOREIGN KEY (`passengerId`) REFERENCES `Passenger` (`passengerId`) ON DELETE CASCADE ON UPDATE CASCADE,
